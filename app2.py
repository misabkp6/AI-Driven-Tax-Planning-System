...
def calculate_tax_liability(data: Dict[str, Any]) -> float:
    ...
    if taxable_income <= 300000:  # Increased from 250000
        tax = 0
...
def render_main_app():
    ...
    with tab1:
        ...
        if calculate_button or "tax_results" in st.session_state:
            # Track that calculate button was clicked
        ...
        if "tax_results" in st.session_state:
            ...
            st.info("👈 Enter your financial details in the sidebar and click 'How to plan my tax' to begin.")
            st.image("https://img.freepik.com/free-vector/tax-concept-illustration_114360-1057.jpg", width=400)

    # Tab 2: Gemini Chat interface
    with tab2:
        # Pass the user's financial data to the chat interface if available
        if "input_data" in st.session_state and "calculate_clicked" in st.session_state and st.session_state.calculate_clicked:
            render_gemini_chat_page(user_data=st.session_state["input_data"])
        else:
